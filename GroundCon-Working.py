...
class socket_manager(QObject):
    ...
    def listening_thread(self):
        while True:
            ...
            for datum in data:
                ...
                # print(Frame_data)
                if  len(self. Frame_data ) == 232 and self. is_checked:
                    read_ptr = 0
    ...
    def send_attitude_command(self, quaternion:list):
        self. TCP_write_thread.acquire()
        try:       
            send_bytes = self.pack_bytes("A", quaternion) 
            self. mysocket.sendto(send_bytes, (self.my_remote_IP, self. my_remote_port_name))
        finally:
            self. TCP_write_thread.release()
    
    def send_force_command(self, torques:list):
        self. TCP_write_thread.acquire()
        try:       
            send_bytes = self.pack_bytes("F", torques) 
            self. mysocket.sendto(send_bytes, (self.my_remote_IP, self. my_remote_port_name))
        finally:
            self. TCP_write_thread.release()
